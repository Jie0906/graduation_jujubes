...
peanut = spectral.envi.open('4-1_processed.hdr').asarray()
#gt = scipy.io.loadmat('peanut_gt.mat') 

xx = [49,89,145]
false_color_bands = np.array(xx)

img = peanut[..., false_color_bands]

plt.figure()
# plt.imshow(peanut[:,:,133])
plt.imshow(img)

#%% 

peanut = spectral.envi.open('1_New-1.hdr').asarray()
gt = scipy.io.loadmat('peanut_gt.mat') 
...
#get ground_truth
plt.figure()
plt.imshow(gt['peanut'])
d_point = plt.ginput(2, timeout = 300)
for i in range(len(d_point)):
    d_point[i] = peanut[int(d_point[i][1]),int(d_point[i][0])]
    d_point_arr.append(d_point[i])
plt.close()

for i in range(len(d_point)):
   cem_temp = cem(peanut,d_point[i])
   namd_temp = namd(peanut,d_point[i])
   amd_temp = amd(peanut,d_point[i])
   lrt_temp = lrt(peanut,d_point[i])
   nlrt_temp = nlrt(peanut,d_point[i])
   snr_temp = snr(peanut,d_point[i])
   hcem_temp = hcem(peanut,d_point[i])
...
hcem_result = np.stack((hcem_result[0],hcem_result[1]),axis=-1)   

np.save('peanut_cem_result.npy', cem_result)
np.save('peanut_namd_result.npy', namd_result)
np.save('peanut_amd_result.npy', amd_result)
np.save('peanut_lrt_result.npy', lrt_result)
np.save('peanut_nlrt_result.npy', nlrt_result)
np.save('peanut_snr_result.npy', snr_result)
np.save('peanut_hcem_result.npy', hcem_result)
...
plt.figure()
plt.imshow(gt['peanut'])
d_point = plt.ginput(1, timeout = 300)
...
plt.figure()
plt.imshow(gt['peanut'])
no_d_point = plt.ginput(1, timeout = 300)
plt.close()

for i in range(len(d_point)):
    tcimf_temp = tcimf(peanut,d_point_arr[i],no_d_point[i])   
    print('processed  ' + str(i) + ' point')
   
    tcimf_result.append(tcimf_temp)

   
tcimf_result = np.stack((tcimf_result[0],tcimf_result),axis=-1) 
  

np.save('peanut_tcimf_result.npy', tcimf_result)
...
peanut_cem_result = np.load('peanut_cem_result.npy',allow_pickle=True)
peanut_namd_result = np.load('peanut_namd_result.npy',allow_pickle=True)
peanut_amd_result = np.load('peanut_amd_result.npy',allow_pickle=True)
peanut_lrt_result = np.load('peanut_lrt_result.npy',allow_pickle=True)
peanut_nlrt_result = np.load('peanut_nlrt_result.npy',allow_pickle=True)
peanut_snr_result = np.load('peanut_snr_result.npy',allow_pickle=True)
peanut_hcem_result = np.load('peanut_hcem_result.npy',allow_pickle=True)

windowSize = 25

peanut_cem_result = np.reshape(peanut_cem_result, (216, 409 ,2))
peanut_namd_result = np.reshape(peanut_namd_result, (216, 409 ,2))
peanut_amd_result = np.reshape(peanut_amd_result, (216, 409 ,2))
peanut_lrt_result = np.reshape(peanut_lrt_result, (216, 409 ,2))
peanut_nlrt_result = np.reshape(peanut_nlrt_result, (216, 409 ,2))
peanut_snr_result = np.reshape(peanut_snr_result, (216, 409 ,2))
peanut_hcem_result = np.reshape(peanut_hcem_result, (216, 409 ,2))

peanut_cem_result_slice, gt_slice = createImageCubes(peanut_cem_result, gt['peanut'], windowSize=windowSize)
peanut_namd_result_slice, gt_slice = createImageCubes(peanut_namd_result, gt['peanut'], windowSize=windowSize)
peanut_amd_result_slice, gt_slice = createImageCubes(peanut_amd_result, gt['peanut'], windowSize=windowSize)
peanut_lrt_result_slice, gt_slice = createImageCubes(peanut_lrt_result, gt['peanut'], windowSize=windowSize)
peanut_nlrt_result_slice, gt_slice = createImageCubes(peanut_nlrt_result, gt['peanut'], windowSize=windowSize)
peanut_snr_result_slice, gt_slice = createImageCubes(peanut_snr_result, gt['peanut'], windowSize=windowSize)
peanut_hcem_result_slice, gt_slice = createImageCubes(peanut_hcem_result, gt['peanut'], windowSize=windowSize)

np.save('gt.npy',gt['peanut'])
np.save('peanut_cem_result_slice.npy',peanut_cem_result_slice)
np.save('peanut_namd_result_slice.npy',peanut_namd_result_slice)
np.save('peanut_amd_result_slice.npy',peanut_amd_result_slice)
np.save('peanut_lrt_result_slice.npy',peanut_lrt_result_slice)
np.save('peanut_nlrt_result_slice.npy',peanut_nlrt_result_slice)
np.save('peanut_snr_result_slice.npy',peanut_snr_result_slice)

#%% model try1

model = Sequential()
model.add(Conv2D(filters=16, kernel_size=3, input_shape=(25, 25, 2), activation='relu', padding = 'same'))
...
model = Sequential()
model.add(Conv2D(filters=32, kernel_size=3, input_shape=(25, 25, 2), activation='relu', padding = 'same'))
model.add(MaxPool2D(pool_size=2))
model.add(Conv2D(filters=64, kernel_size=3, activation='relu', padding = 'same'))
model.add(MaxPool2D(pool_size=2))
model.add(Conv2DTranspose(filters=64, kernel_size=3,activation='relu', padding='same'))
...
def cov2D(cnn_filter, counting_run):
    acc_list  = []
    acc_list_top5 = []
    origin_train, origin_test, ans_train, ans_test = train_test_split( cnn_filter, gt_slice, test_size = 0.75, random_state = 0)
    ...
    #save model
    model.save("peanut_namd_model.h5")
    ...
    dataset = "peanut"
    # load best weights
    model.load_weights("peanut_namd_model.h5")
    ...
    file_name = "peanut_namd_detect.txt"
...
#%% main
cov2D(peanut_namd_result_slice, 1)
